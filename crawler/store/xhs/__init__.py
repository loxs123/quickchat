...
async def update_xhs_note(note_item: Dict):
    ...
    # 显示界面
    head_md = f"### [{local_db_item['title']}]({local_db_item['note_url']})"

    image_list = local_db_item['image_list'].split(',')
    image_md = '<div style="overflow-x: auto;">\n<table>\n<tr>\n'
    for i in range(len(image_list)):
        # if i % 3 == 0 and i != 0:
        #     html += '</tr>\n<tr>\n'
        image_md += f'<td><img src="https://images.weserv.nl/?url={image_list[i]}&w=300&h=600"/></td>\n'
    image_md += '</tr>\n</table>\n</div>'

    content_md = local_db_item['desc']
    video_url = local_db_item['video_url']
    
    if video_url != '':
        content_md = f'{content_md}\n视频链接:{video_url}'
    time1 = local_db_item['time']
    time2 = local_db_item['last_update_time']
    if time1 != 0:
        time1 = datetime.datetime.fromtimestamp(time1 / 1000).strftime('%Y-%m-%d %H:%M:%S')
        content_md = f'{content_md}\n发布时间:{time1}'
    if time2 != 0:
        time2 = datetime.datetime.fromtimestamp(time2 / 1000).strftime('%Y-%m-%d %H:%M:%S')
        content_md = f'{content_md}\n更新时间:{time2}'
