...
class TieBaCrawler(AbstractCrawler):
    ...
    async def search(self, **kwargs) -> str:
        ...
        utils.logger.info(f"[BaiduTieBaCrawler.search] Current search keyword: {config.KEYWORDS}")
        page = 1
        while note_cnt < config.CRAWLER_MAX_NOTES_COUNT:
            if page < start_page:
                ...
                continue
            try:
                utils.logger.info(f"[BaiduTieBaCrawler.search] search tieba keyword: {config.KEYWORDS}, page: {page}")
                ...
                async for md in self.get_specified_notes(note_id_list):
                    yield md
                    note_cnt += 1
    ...
    async def get_specified_notes(self, note_id_list: List[str] = config.TIEBA_SPECIFIED_ID_LIST):
        ...
        for note_id in note_id_list:
            note_detail = await self.get_note_detail_async_task(note_id=note_id, semaphore=semaphore)
            if note_detail is not None:
                md = await tieba_store.update_tieba_note(note_detail)
                yield md
